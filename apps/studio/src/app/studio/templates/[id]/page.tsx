...
import type { SlideTemplateInfo, PropSchema } from "@/lib/studio/slide-templates";
...
function SlideTemplateDetail({ tpl }: { tpl: SlideTemplateInfo }) {
  ...
  // Separate content vs style schema entries
  const SKIP_KEYS = new Set(["slideNumber", "backgroundColor", "accentColor", "textColor", "mutedColor"]);
  const contentEntries = Object.entries(tpl.propsSchema).filter(
    ([key, s]) => !SKIP_KEYS.has(key) && (s.group ?? "content") === "content"
  );
  const styleEntries = Object.entries(tpl.propsSchema).filter(
          ...
          <div className="bg-[#0f0f0f] border border-[#222] rounded-xl p-5">
            ...
            <div className="space-y-3">
              ...
              {activeTab === "style" && styleEntries.length === 0 && (
                <p className="text-xs text-[#444] italic py-4 text-center">이 템플릿에는 스타일 옵션이 없습니다</p>
              )}
            </div>
          </div>
