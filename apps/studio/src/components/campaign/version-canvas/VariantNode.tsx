...
const AVATAR_FALLBACK = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 40 40'%3E%3Ccircle cx='20' cy='20' r='20' fill='%23333'/%3E%3C/svg%3E";

/* Split text into thread segments by --- separator */
...
/* ── Single Threads post ── */
function ThreadsPost({ text, isFirst, isLast, index }: { text: string; isFirst: boolean; isLast: boolean; index: number }) {
  return (
    <div className={`p-3 ${!isLast ? "pb-0" : ""}`}>
      <div className="flex gap-2.5">
        {/* Avatar + thread line */}
        <div className="flex flex-col items-center flex-shrink-0">
          <img src={AVATAR} alt="" className="w-9 h-9 rounded-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = AVATAR_FALLBACK; }} />
          {!isLast && <div className="w-[2px] flex-1 bg-[#333] mt-1.5 mb-0 min-h-[8px]" />}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-1">
            <span className="text-[12px] font-semibold text-[#fafafa]">brxce.ai</span>
            {isFirst && <span className="text-[10px] text-[#666]">· 방금</span>}
            {!isFirst && <span className="text-[9px] text-[#555] font-mono">#{index + 1}</span>}
          </div>
          <p className="text-[11px] text-[#e0e0e0] whitespace-pre-wrap break-words leading-[1.5]">
            {text}
          </p>
          {isLast && <ThreadsActions />}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-[#101010] w-full">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#222]">
        ...
        {isThread && <span className="text-[9px] text-[#555] ml-auto">{segments.length}개 스레드</span>}
      </div>
      {segments.map((seg, i) => (
        <ThreadsPost
          key={i}
