...
export function CTAChecklistScore({ title, ranges, footer, prompt, ...colors }: CTAChecklistScoreProps) {
  ...
  return (
    <SlideBase {...colors}>
      ...
      >
        <SlideTitle variant="title" style={{ fontSize: fontSize.headingLg, fontWeight: fontWeight.bold, lineHeight: lineHeight.default, textAlign: 'center' as const }}>
          {title}
        </SlideTitle>
          ...
          {ranges.map((r, idx) => (
            ...
            >
              ...
              <div style={{ textAlign: 'left' as const, flex: 1 }}>
                <span style={{ fontSize: fontSize.bodySm, fontWeight: fontWeight.bold, color: accent, display: 'block' }}>
                  {r.range}
                </span>
                <MutedText size="md" mutedColor={colors.textColor || '#f5f5f5'} style={{ fontSize: fontSize.bodyXs, fontWeight: fontWeight.medium }}>
                  {r.label}
                </MutedText>
              </div>
            </div>
        ...
        {footer && (
          <MutedText size="lg" mutedColor={colors.textColor || '#f5f5f5'} style={{ marginTop: gap['3xl'], fontSize: fontSize.bodySm, fontWeight: fontWeight.semibold }}>
            {footer}
          </MutedText>
        )}
