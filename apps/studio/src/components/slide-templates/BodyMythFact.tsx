...
export function BodyMythFact({ myth, fact, mythLabel, factLabel, ...colors }: BodyMythFactProps) {
  ...
  return (
    <SlideBase {...colors}>
      <div
        ...
        style={{ paddingLeft: spacing.safeX, paddingRight: spacing.safeX, paddingTop: spacing.safeY, paddingBottom: spacing.safeY, gap: gap['4xl'] }}
      >
        {/* Myth */}
        <div>
          ...
          <MutedText size="lg" mutedColor={muted} style={{ fontSize: fontSize.bodyMd, lineHeight: lineHeight.relaxed, whiteSpace: 'pre-line' as const, paddingLeft: 68 }}>
            {myth}
          </MutedText>
        </div>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: gap.lg, marginBottom: gap.lg }}>
            ...
            <span style={{ fontSize: fontSize.captionLg, fontWeight: fontWeight.bold, color: accent, letterSpacing: '0.15em', textTransform: 'uppercase' as const }}>
              {factLabel || 'FACT'}
            </span>
          </div>
          <MutedText size="lg" mutedColor={colors.textColor || '#f5f5f5'} style={{ fontSize: fontSize.bodyMd, fontWeight: fontWeight.semibold, lineHeight: lineHeight.relaxed, whiteSpace: 'pre-line' as const, paddingLeft: 68 }}>
            {fact}
          </MutedText>
        </div>
      </div>
