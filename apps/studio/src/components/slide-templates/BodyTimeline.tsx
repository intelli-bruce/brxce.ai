...
export function BodyTimeline({ title, items, ...colors }: BodyTimelineProps) {
  const accent = colors.accentColor || '#ff6b35'
  const muted = colors.mutedColor || '#a3a3a3'
  const text = colors.textColor || '#f5f5f5'
        ...
        <div style={{ display: 'flex', flexDirection: 'column', position: 'relative', paddingLeft: 36 }}>
          ...
          {normalizedItems.map((item, idx) => (
            ...
            >
              ...
              <div>
                {item.label && (
                  <span style={{ fontSize: fontSize.captionLg, fontWeight: fontWeight.bold, color: accent, display: 'block', marginBottom: gap.xs }}>
                    {item.label}
                  </span>
                )}
                <MutedText size="lg" mutedColor={text} style={{ fontSize: fontSize.bodyMd, fontWeight: fontWeight.medium, lineHeight: lineHeight.relaxed }}>
                  {item.desc}
                </MutedText>
              </div>
            </div>
