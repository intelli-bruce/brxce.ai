...
export function CTAShare({ title, targetPerson, reason, prompt, ...colors }: CTAShareProps) {
  ...
  return (
    <SlideBase {...colors}>
      ...
      >
        ...
        >
          <MutedText size="lg" mutedColor={colors.textColor || '#f5f5f5'} style={{ fontSize: fontSize.bodyMd, fontWeight: fontWeight.semibold, lineHeight: lineHeight.relaxed, whiteSpace: 'pre-line' as const }}>
            {targetPerson}
          </MutedText>
        </div>

        {reason && (
          <MutedText size="lg" mutedColor={colors.mutedColor || '#a3a3a3'} style={{ marginTop: gap['2xl'], fontSize: fontSize.bodySm, lineHeight: lineHeight.relaxed }}>
            {reason}
          </MutedText>
        )}
